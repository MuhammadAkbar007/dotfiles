# Generated by Powerlevel10k configuration wizard on 2026-08-13 at 09:06 +05.
...
() {
  ...
  typeset -g POWERLEVEL9K_RIGHT_PROMPT_ELEMENTS=(
    ...
    python_version          # python version (custom segment, see prompt_python_version below)
  ...
  # p10k has no built-in python_version segment (only pyenv/virtualenv/anaconda), so define one.
  # Mirrors POWERLEVEL9K_NODE_VERSION_PROJECT_ONLY: shown only inside a python project tree.
  # ponytail: runs `python3 -V` once per prompt in python dirs only; cache if it ever drags.
  function prompt_python_version() {
    local dir=$PWD
    while [[ $dir != / ]]; do
      [[ -e $dir/pyproject.toml || -e $dir/requirements.txt || -e $dir/setup.py ]] && break
      dir=${dir:h}
    done
    [[ $dir == / ]] && return
    local v=${${(z)"$(python3 -V 2>&1)"}[2]}
    [[ -n $v ]] && p10k segment -f 37 -r -i PYTHON_ICON -t "${v}"
  }
